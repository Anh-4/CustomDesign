...
export default function App() {
  ...
  const generate = () => {
    ...
    // Tạo NUM_OPTIONS mẫu SONG SONG — mẫu nào xong hiện trước, lỗi 1 mẫu không chặn mẫu kia.
    for (let i = 0; i < NUM_OPTIONS; i++) {
      const idx = i;
      const prompt = buildCustomPrompt(spec, idx);
      Flow.generate.image({ prompt, model: effectiveModel, provider, referenceImageMediaIds: refs })
        .then((out) => {
          setResults((prev) => {
            const next = [...prev];
          ...
          <div className="flex-1 flex items-center justify-center p-6">
            <div className="flex flex-col items-center gap-3 text-white/25 max-w-[360px] text-center">
              ...
              <span className="text-[13px] leading-relaxed">
                Tải <span className="text-white/50">design gốc (ô 1)</span>, rồi nhập text/số hoặc tải ảnh thay thế (ô 2–4).
                AI giữ nguyên style gốc, chỉ thay phần Anh4 chỉ định và tạo {NUM_OPTIONS} mẫu để chọn.
              </span>
            </div>
          </div>
        )}
